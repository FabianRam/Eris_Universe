...
private var obstacleOb:GameObject;
...
function addObsticle() { 
	...
	var x1 = transform.position.x - GetComponent.<Renderer>().bounds.size.x*3/10;
    var x2 = transform.position.x + GetComponent.<Renderer>().bounds.size.x*3/10;
	...
	{
		if(obstacle2!=null)
		obstacleOb=obstacle2;
		else
		obstacleOb=obstacle;
		 Instantiate(obstacleOb, spawnPoint, Quaternion.identity);
	}
	else
	{
			if(obstacle3!=null)
		obstacleOb=obstacle3;
		else
		obstacleOb=obstacle;
		 Instantiate(obstacleOb, spawnPoint, Quaternion.identity);
	}
